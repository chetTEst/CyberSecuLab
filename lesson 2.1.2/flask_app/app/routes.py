...
from flask import render_template, request, redirect, url_for, flash, Markup, jsonify, session, abort, send_from_directory
...
def dashboard(session_number):
    ...
    ips = [{'id': q.id, 'number': q.number, 'value': q.value, 'isDanger': q.isDanger} for q in questions]  # не включаем 'answer'
