# -*- cuding: utf-8 -*-
import random
from .models import db, User, Ports, Questions
from . import app
    ...
    with app.app_context():
        ...
        questionsIsDangerPort = ic(db.session.query(Ports.id).filter(
            and_(Ports.isDanger == True,
                 Ports.isAnswer == False)).all())
        salt_lang_data = db.session.query(Questions.id).all()
        random.shuffle(salt_lang_data)
        ...
        user = User(user_id=user_id, flag=random.choice(flags),
                    ...
                    q8=questiosShufflePorts[7], q9=Questions.query.get(salt_lang_data[0][0]))
        db.session.add(user)
        db.session.commit()
