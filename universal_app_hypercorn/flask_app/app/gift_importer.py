    ...
    def parse(self) -> int:
        ...
        try:
            for block in content.split("\n\n"):
                ...
                clean_text = self._clean(qtext)
                body = qmatch.group("body").strip()

                if self._persist_question(clean_text, body, pool):
                    inserted += 1
            ...
            for opt_text, is_correct in opts:
                if is_correct:
                    try:
                        ...
                        # Логируем ошибку и продолжаем с простым текстом
                        print(f"Ошибка хеширования: {e}, используем текст как есть")
                else:
