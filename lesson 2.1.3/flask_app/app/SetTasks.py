...
def setTasks():
    dataTasks = [{
                 ...
                 {
                     'text': '<p>У вас есть поле ввода, для поиска по типу деревьев в нашей базе данных. Ниже вы видите запрос к БД. Используя поле запроса, осуществите SQL иньекцию для получения всех записей БД</p><p><i>Запрос создан, он заблокирован. Изменить его сейчас нельзя. Можно редактировать только поле поискгового запроса.</i><Структура запроса:</p><p class="font-weight-light text-monospace">ЧТОСДЕЛАТЬ для_каких_полей ИЗ_какого_Хранилища Условие = [ЗДЕСЬ БУДЕТ ВАЩ ЗАПРОС КАК ЕСТЬ];</p>',
                     'number': 6,
                ...
                {
                    'text': '<p>У вас есть поле ввода, для поиска по типу деревьев в нашей базе данных, в котором уже указана SQL иньекция. Исправьте запрос так, чтобы иньекция не удалась, для этого используйте параметрические запросы. Каждый параметр указывается не как есть, а в виде знака <u><b>?</b></u></p><p><i>Запрос разблокирован. Теперь изменить нельзя поисковый запрос в котором указана SQL иньекция. Можно редактировать только запроса к базе данных.</i><Структура запроса:</p><p class="font-weight-light text-monospace">ЧТОСДЕЛАТЬ для_каких_полей ИЗ_какого_Хранилища Условие = Знак_вопроса;</p>',
                    'number': 7,
